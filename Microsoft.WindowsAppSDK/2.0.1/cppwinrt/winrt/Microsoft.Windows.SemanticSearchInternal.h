...
namespace winrt::impl
{
    ...
    template <typename D> auto consume_Microsoft_Windows_SemanticSearchInternal_ITokenizerSession3<D>::Tokenize(uint64_t maxTokenCount, param::hstring const& text, array_view<int64_t> tokens, uint64_t& tokensUsed, array_view<uint64_t> indices, uint64_t& indicesUsed, uint32_t priority, uint32_t frequency, bool mapBackToUtf16) const
    {
        if constexpr (!std::is_same_v<D, winrt::Microsoft::Windows::SemanticSearchInternal::ITokenizerSession3>)
        {
            winrt::hresult _winrt_cast_result_code;
            auto const _winrt_casted_result = impl::try_as_with_reason<winrt::Microsoft::Windows::SemanticSearchInternal::ITokenizerSession3, D const*>(static_cast<D const*>(this), _winrt_cast_result_code);
            check_hresult(_winrt_cast_result_code);
            auto const _winrt_abi_type = *(abi_t<winrt::Microsoft::Windows::SemanticSearchInternal::ITokenizerSession3>**)&_winrt_casted_result;
            check_hresult(_winrt_abi_type->Tokenize(maxTokenCount, *(void**)(&text), tokens.size(), put_abi(tokens), &tokensUsed, indices.size(), put_abi(indices), &indicesUsed, priority, frequency, mapBackToUtf16));
        }
        else
        {
            auto const _winrt_abi_type = *(abi_t<winrt::Microsoft::Windows::SemanticSearchInternal::ITokenizerSession3>**)this;
            check_hresult(_winrt_abi_type->Tokenize(maxTokenCount, *(void**)(&text), tokens.size(), put_abi(tokens), &tokensUsed, indices.size(), put_abi(indices), &indicesUsed, priority, frequency, mapBackToUtf16));
        }
    }
    template <typename D>
    ...
    {
        int32_t __stdcall Tokenize(uint64_t maxTokenCount, void* text, uint32_t __tokensSize, int64_t* tokens, uint64_t* tokensUsed, uint32_t __indicesSize, uint64_t* indices, uint64_t* indicesUsed, uint32_t priority, uint32_t frequency, bool mapBackToUtf16) noexcept final try
        {
            typename D::abi_guard guard(this->shim());
